...
__global__
void compress_kernel(int *parent, int *result, int size){
...
void compress_gpu(std::vector<int> &parent){
  ...
  // copy back
  cudaMemcpy(parent.data(), d_result, sizeof(int) * num_bytes, cudaMemcpyDeviceToHost);
  cudaFree(d_parent);
  cudaFree(d_result);
}
